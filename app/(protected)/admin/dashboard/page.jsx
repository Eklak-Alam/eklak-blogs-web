...
import { 
  ...
  CheckCircle2, XCircle, FileText, CheckSquare, BarChart3, Database,
  MoreHorizontal, Calendar, Mail, Check, ChevronDown, PenTool, ExternalLink,
  Edit3
} from "lucide-react";
...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    if (isInitialized) {
      if (!isAuthenticated) router.push("/login");
        ...
        toast.error("Security Breach: You do not have Admin clearance.");
  ...
  // Users
  const userFilters = { page: userPage, limit: 10, ...(debouncedUserSearch && { search: debouncedUserSearch }), ...(userRoleFilter && { role: userRoleFilter }) };
  ...
  const handleRoleChange = (userId, newRole) => {
    if (userId === currentUser.id) return toast.error("Cannot change your own role.");
    updateRole({ id: userId, payload: { role: newRole } });
  };
  const handleToggleBan = (userId, currentBanStatus, userName) => {
    if (userId === currentUser.id) return toast.error("Cannot ban yourself.");
    if (window.confirm(`Are you sure you want to ${currentBanStatus ? "unban" : "ban"} ${userName}?`)) {
      toggleBan({ id: userId, payload: { isBanned: !currentBanStatus } });
    }
  };
  const handleDeleteUser = (userId, userName) => {
    if (userId === currentUser.id) return toast.error("Cannot delete your own admin account.");
    if (window.confirm(`CRITICAL: Permanently delete ${userName} and all their posts?`)) {
      deleteUser(userId);
    }
  };
  ...
  const handleDeletePost = (postId, title) => {
    if (window.confirm(`Force delete post: "${title}"?`)) {
      deletePost(postId);
    }
  };
    ...
    <div className="min-h-screen bg-[var(--color-background)] relative selection:bg-[var(--color-brand-primary)]/30">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden h-[50vh]">
        ...
        <motion.div 
          ...
          className="absolute top-0 right-1/4 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-[var(--color-brand-accent)] rounded-full blur-[140px]"
        />
      </div>
        ...
        >
          <div>
            ...
            <div className="flex items-center gap-3">
              <Shield className="w-10 h-10 text-[var(--color-brand-accent)]" />
            ...
            <p className="text-[var(--color-muted)] text-lg font-medium mt-2">
              Global platform analytics, user moderation, and bulk content management.
            </p>
          </div>
        </motion.div>
            ...
            >
              {(statsLoading1 || statsLoading2) ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {/* User Stats */}
                  <div className="p-6 rounded-3xl bg-gradient-to-br from-[var(--color-surface)] to-[var(--color-background)] border border-[var(--color-border)] shadow-sm hover:border-[var(--color-brand-primary)]/50 transition-all">
                    <p className="text-xs font-bold text-[var(--color-muted)] uppercase tracking-widest mb-2">Total Users</p>
                    <p className="text-4xl font-extrabold text-[var(--color-foreground)] mb-4">{userStats.totalUsers || 0}</p>
                    <div className="flex items-center gap-2 text-xs font-bold bg-green-500/10 text-green-600 px-3 py-1 rounded-full w-fit">
                      +{userStats.newUsersLast30Days || 0} in 30 days
                    </div>
                  </div>
                  ...
                  <div className="p-6 rounded-3xl bg-[var(--color-surface)] border border-[var(--color-border)] shadow-sm col-span-1 lg:col-span-3 flex items-center justify-around flex-wrap gap-4">
                    <div className="text-center">
                      <p className="text-2xl font-bold text-purple-500">{userStats.roleDistribution?.ADMIN || 0}</p>
                      <p className="text-xs font-bold text-[var(--color-muted)] uppercase tracking-wider">Admins</p>
                    </div>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-blue-500">{userStats.roleDistribution?.AUTHOR || 0}</p>
                      <p className="text-xs font-bold text-[var(--color-muted)] uppercase tracking-wider">Authors</p>
                    </div>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-emerald-500">{userStats.roleDistribution?.WRITER || 0}</p>
                      <p className="text-xs font-bold text-[var(--color-muted)] uppercase tracking-wider">Writers</p>
                    </div>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-[var(--color-foreground)]">{userStats.roleDistribution?.USER || 0}</p>
                      <p className="text-xs font-bold text-[var(--color-muted)] uppercase tracking-wider">Users</p>
                    </div>
                  </div>
          ...
          {activeTab === "USERS" && (
            ...
            >
              ...
              <div className="flex flex-col lg:flex-row gap-6 mb-8 items-start lg:items-center justify-between">
                ...
                <div className="flex gap-2 bg-[var(--color-surface)]/60 backdrop-blur-md p-1.5 rounded-2xl border border-[var(--color-border)]/50 overflow-x-auto w-full lg:w-auto">
                  {[
                    { id: "", label: "All Users" },
                    { id: "USER", label: "Readers" },
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 px-6 py-2 rounded-xl text-xs font-bold tracking-wide transition-all whitespace-nowrap ${
                ...
                <div className="relative w-full lg:w-80">
                  ...
                  <input 
                    type="text" placeholder="Search by name or email..." value={userSearch} onChange={(e) => setUserSearch(e.target.value)}
                    className="w-full bg-[var(--color-surface)]/60 backdrop-blur-md border border-[var(--color-border)] rounded-2xl pl-10 pr-4 py-3 text-sm font-medium outline-none focus:border-[var(--color-brand-primary)] transition-all shadow-sm"
                  />
                </div>
              </div>

              {/* Table Container */}
              <div className="bg-[var(--color-surface)]/40 backdrop-blur-xl border border-[var(--color-border)]/60 rounded-3xl shadow-2xl overflow-visible relative min-h-[500px]">
                {usersLoading && <div className="absolute inset-0 bg-black/5 z-20 flex items-center justify-center backdrop-blur-sm rounded-3xl"><Loader2 className="w-8 h-8 animate-spin text-[var(--color-brand-primary)]" /></div>}
                
                <div className="overflow-x-auto overflow-y-visible">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-[var(--color-surface)]/80 border-b border-[var(--color-border)]">
                        <th className="p-5 text-xs font-extrabold text-[var(--color-muted)] uppercase tracking-widest whitespace-nowrap">User Identity</th>
                        <th className="p-5 text-xs font-extrabold text-[var(--color-muted)] uppercase tracking-widest whitespace-nowrap">System Role</th>
                    ...
                    <tbody className="divide-y divide-[var(--color-border)]/30">
                      <AnimatePresence mode="popLayout">
                        {usersList.length === 0 && !usersLoading ? (
                          <tr><td colSpan="4" className="p-12 text-center text-[var(--color-muted)] font-medium">No users found.</td></tr>
                        ) : (
                          usersList.map((usr) => (
                            <React.Fragment key={usr.id}>
                              <motion.tr 
                                ...
                                className={`transition-colors group cursor-pointer ${expandedUserId === usr.id ? 'bg-[var(--color-surface)]' : 'hover:bg-[var(--color-surface)]/50'}`}
                              >
                                <td className="p-5">
                                  <div className="flex items-center gap-4">
                                    <div className="relative">
                                      {usr.image ? (
                                        <img src={usr.image} alt="" className="w-12 h-12 rounded-full object-cover border-2 border-[var(--color-border)] shadow-sm" />
                                      ) : (
                                        <div className="w-12 h-12 rounded-full bg-[var(--color-brand-dark)] text-white flex items-center justify-center font-bold text-lg shadow-sm">
                                          {usr.name.charAt(0).toUpperCase()}
                                        </div>
                                      )}
                                      <div className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 border-[var(--color-background)] ${usr.isBanned ? 'bg-red-500' : 'bg-green-500'}`} />
                                    </div>
                                    <div>
                                      <p className="text-sm font-extrabold text-[var(--color-foreground)] line-clamp-1 group-hover:text-[var(--color-brand-primary)] transition-colors">{usr.name}</p>
                                      <p className="text-xs font-medium text-[var(--color-muted)] line-clamp-1">{usr.email}</p>
                                    </div>
                                  </div>
                                </td>
                                <td className="p-5">
                                  <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-extrabold tracking-widest uppercase border ${
                                    usr.role === 'ADMIN' ? 'bg-purple-500/10 text-purple-500 border-purple-500/20' :
                                    usr.role === 'AUTHOR' ? 'bg-blue-500/10 text-blue-500 border-blue-500/20' :
                                    usr.role === 'WRITER' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' :
                                    'bg-[var(--color-surface)] text-[var(--color-muted)] border-[var(--color-border)]'
                                  }`}>
                                    {usr.role === 'ADMIN' && <Shield className="w-3 h-3" />}
                                    {usr.role === 'AUTHOR' && <PenTool className="w-3 h-3" />}
                                    {usr.role === 'WRITER' && <FileText className="w-3 h-3" />}
                                    {usr.role}
                                  </span>
                                </td>
                                <td className="p-5">
                                  {usr.isBanned ? (
                                    <span className="inline-flex items-center gap-1.5 text-red-500 text-xs font-bold"><XCircle className="w-4 h-4" /> Suspended</span>
                                  ) : (
                                    <span className="inline-flex items-center gap-1.5 text-green-500 text-xs font-bold"><CheckCircle2 className="w-4 h-4" /> Active</span>
                                  )}
                                </td>
                                <td className="p-5 text-right relative">
                                  <button 
                                    onClick={(e) => { e.stopPropagation(); setActiveMenuId(activeMenuId === usr.id ? null : usr.id); }}
                                    className="p-2 text-[var(--color-muted)] hover:text-[var(--color-foreground)] hover:bg-[var(--color-border)]/50 rounded-xl transition-all"
                                  >
                                  ...
                                  <AnimatePresence>
                                    {activeMenuId === usr.id && (
                                      <motion.div 
                                        ...
                                        className="absolute right-8 top-12 w-48 bg-[var(--color-background)]/90 backdrop-blur-xl border border-[var(--color-border)] rounded-2xl shadow-2xl z-50 overflow-hidden text-left"
                                        onClick={(e) => e.stopPropagation()}
                                      >
                                        <div className="p-2 border-b border-[var(--color-border)]/50">
                                          <p className="text-[10px] font-extrabold text-[var(--color-muted)] uppercase tracking-widest px-2 mb-1">Update Role</p>
                                          {["USER", "WRITER", "AUTHOR", "ADMIN"].map(role => (
                                            <button 
                                              ...
                                              className="w-full text-left px-3 py-2 text-xs font-bold text-[var(--color-foreground)] hover:bg-[var(--color-surface)] rounded-xl transition-colors flex items-center justify-between disabled:opacity-50"
                                            >
                                              {role} {usr.role === role && <Check className="w-3 h-3 text-[var(--color-brand-primary)]" />}
                                            </button>
                                          ))}
                                        </div>
                                        <div className="p-2">
                                          <button 
                                            ...
                                            className="w-full text-left px-3 py-2 text-xs font-bold text-orange-500 hover:bg-orange-500/10 rounded-xl transition-colors flex items-center gap-2 disabled:opacity-50"
                                          >
                                            <Ban className="w-4 h-4" /> {usr.isBanned ? "Lift Suspension" : "Suspend Account"}
                                          </button>
                                          <button 
                                            onClick={() => { handleDeleteUser(usr.id, usr.name); setActiveMenuId(null); }}
                                            disabled={isDeletingUser || usr.id === currentUser.id}
                                            className="w-full text-left px-3 py-2 text-xs font-bold text-red-500 hover:bg-red-500/10 rounded-xl transition-colors flex items-center gap-2 disabled:opacity-50"
                                          >
                              ...
                              <AnimatePresence>
                                {expandedUserId === usr.id && (
                                  <motion.tr 
                                    ...
                                    className="bg-[var(--color-surface)]/30 border-b border-[var(--color-border)]/50"
                                  >
                                    <td colSpan="4" className="p-0">
                                      <div className="px-8 py-6 flex flex-col md:flex-row gap-8">
                                        <div className="flex-1 space-y-4">
                                          <h4 className="text-xs font-extrabold text-[var(--color-muted)] uppercase tracking-widest border-b border-[var(--color-border)] pb-2 mb-4">User Insights</h4>
                                          <div className="grid grid-cols-2 gap-4">
                                            <div className="flex items-center gap-3">
                                              <div className="w-8 h-8 rounded-full bg-[var(--color-surface)] border border-[var(--color-border)] flex items-center justify-center"><Mail className="w-4 h-4 text-[var(--color-muted)]" /></div>
                                              <div><p className="text-[10px] font-bold text-[var(--color-muted)] uppercase">Contact</p><p className="text-sm font-bold text-[var(--color-foreground)]">{usr.email}</p></div>
                                            </div>
                                            <div className="flex items-center gap-3">
                                              <div className="w-8 h-8 rounded-full bg-[var(--color-surface)] border border-[var(--color-border)] flex items-center justify-center"><Calendar className="w-4 h-4 text-[var(--color-muted)]" /></div>
                                              <div><p className="text-[10px] font-bold text-[var(--color-muted)] uppercase">Joined</p><p className="text-sm font-bold text-[var(--color-foreground)]">{new Date(usr.createdAt).toLocaleDateString()}</p></div>
                                            </div>
                                          </div>
                                        </div>
                                        <div className="w-px bg-[var(--color-border)] hidden md:block" />
                                        <div className="flex-1 flex items-center justify-end">
                                          <button className="px-6 py-2.5 rounded-xl bg-[var(--color-foreground)] text-[var(--color-background)] text-sm font-bold shadow-xl hover:scale-105 transition-transform flex items-center gap-2">
              ...
              {userPagination?.totalPages > 1 && (
                <div className="flex justify-between items-center mt-6 px-2">
                  <span className="text-xs font-bold text-[var(--color-muted)] uppercase tracking-widest">Page {userPage} of {userPagination.totalPages}</span>
                  <div className="flex gap-2">
                    <button onClick={() => setUserPage(p => Math.max(1, p - 1))} disabled={userPage === 1} className="p-3 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-full hover:border-[var(--color-brand-primary)] transition-all disabled:opacity-50"><ChevronLeft className="w-4 h-4" /></button>
                    <button onClick={() => setUserPage(p => Math.min(userPagination.totalPages, p + 1))} disabled={userPage === userPagination.totalPages} className="p-3 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-full hover:border-[var(--color-brand-primary)] transition-all disabled:opacity-50"><ChevronRight className="w-4 h-4" /></button>
                  </div>
                </div>
              )}
            </motion.div>
                            ...
                            >
                              ...
                              <td className="p-5 text-right">
                                <div className="flex items-center justify-end gap-2 opacity-50 md:opacity-0 group-hover:opacity-100 transition-opacity">
                                  <button onClick={() => handleDeletePost(post.id, post.title)} disabled={isDeletingPost} className="p-2 bg-red-500/10 text-red-500 hover:bg-red-500/20 rounded-xl transition-colors disabled:opacity-50" title="Force Delete Post"><Trash2 className="w-4 h-4" /></button>
                                </div>
                              </td>
                            </motion.tr>
                          ))
