...
// OPTIMIZATION: Moved outside the component to prevent recreating the function on every render
...
// OPTIMIZATION: React.memo prevents unnecessary re-renders if the parent component updates
const ProductLaunches = React.memo(function ProductLaunches() {
  const scrollContainerRef = useRef(null);

  // Fetching real data (Updated to fetch Project instead of Product)
  ...
  // OPTIMIZATION: Memoize posts array so it doesn't get recalculated on every render
  const posts = useMemo(() => {
    return postsResponse?.data?.posts || postsResponse?.posts || [];
  }, [postsResponse]);

  // OPTIMIZATION: useCallback preserves the reference to the scroll function
  ...
  // Instead of returning null, we determine if we have posts
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-12 w-full flex flex-col gap-6 md:gap-8">
        ...
        {hasPosts ? (
          <motion.div 
            ...
            className="flex gap-6 md:gap-8 overflow-x-auto snap-x snap-mandatory pb-8 pt-2 scrollbar-hide [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden scroll-smooth"
          >
            {posts.map((post) => (
              <div 
                key={post.id} 
          ...
          <div className="w-full py-12 flex flex-col items-center justify-center text-center bg-zinc-200/30 rounded-2xl border border-zinc-200/80">
             <p className="text-zinc-500 font-medium">No project launches found yet.</p>
             <p className="text-zinc-400 text-sm mt-1">Check back later or ensure the API is running correctly.</p>
          </div>
        )}
